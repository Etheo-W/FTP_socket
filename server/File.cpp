...
int File::getFileSize(FILE_OPERATE_STYLE style)
{
	...
	this->fileSize = (uint32_t)ftell(this->fp);	//ftell用于得到文件位置指针当前位置相对于文件首的偏移字节数；
										//2.1G以上的文件操作时可能出错
	this->closeFile();
...
{
    getcwd(this->fileFullPath, MAX_PATH);
	strcat(this->fileFullPath, "/SocketServerFiles/");
    if (access(this->fileFullPath, 0) < 0) {
        mkdir(this->fileFullPath, S_IRWXU);
    }
...
{
	return (int)fread(str, sizeof(char), n, this->fp);
}

int File::writeFile(char* str, int n)
{
	return (int)fwrite(str, sizeof(char), n, this->fp);
}
